...
# Create directories
for d in ['/workspace/huggingface_cache', 
...
target_model = AutoModelForCausalLM.from_pretrained(target_checkpoint,
                                device_map='auto',
                                torch_dtype=torch.float16)
qwen_model = AutoModelForCausalLM.from_pretrained(qwen_checkpoint,
                                device_map='auto',
                                torch_dtype=torch.float16)
llama_assistant_model = AutoModelForCausalLM.from_pretrained(llama_assistant_checkpoint,
                                device_map='auto',
                                torch_dtype=torch.float16)
llama_3b_assistant_model = AutoModelForCausalLM.from_pretrained("meta-llama/Llama-3.2-3B-Instruct",
                                device_map='auto',
                                torch_dtype=torch.float16)
...
def generate_assisted(prompt, target_model, assistant_model, target_tokenizer,
            assistant_tokenizer=None, do_sample=False):
...
for i, example in enumerate(dataset_sample):
    ...
    # Baseline generation
    baseline_text, baseline_time_per_token, baseline_prefill_time, \
        baseline_inp_len, baseline_out_len = generate_baseline(
            prompt, target_model, target_tokenizer)
    # Assisted generation
    qwen_text, qwen_time_per_token, qwen_prefill_time, \
        qwen_inp_len, qwen_out_len = generate_assisted(
                                                    prompt, target_model,
                                                    qwen_model,
                                                    target_tokenizer,
                                                    qwen_tokenizer, do_sample=True)

    qwen_uag_text, qwen_uag_time_per_token, qwen_uag_prefill_time, \
        qwen_uag_inp_len, qwen_uag_out_len = generate_assisted(
                                                    prompt, target_model,
                                                    qwen_model,
                                                    target_tokenizer,
                                                    qwen_tokenizer, do_sample=False)
    # Assisted generation
    llama_assisted_text, llama_assisted_time_per_token, llama_assisted_prefill_time, \
        llama_assisted_inp_len, llama_assisted_out_len = generate_assisted(
                                                    prompt, target_model,
                                                    llama_assistant_model,
                                                    target_tokenizer,
                                                    None)
    llama_3b_assisted_text, llama_3b_assisted_time_per_token, llama3b__assisted_prefill_time, \
        llama_3b_assisted_inp_len, llama_3b_assisted_out_len = generate_assisted(
                                                    prompt, target_model,
                                                    llama_3b_assistant_model,
                                                    target_tokenizer,
                                                    None)
    # Store results
    results.append({
        ...
        "Llama 1B Len Out": qwen_uag_out_len,
